...
from projects.models import Projects, ProjectTags, ProjectImages, ProjectRating, Comments, Replies, ReportProject, \
    ReportComment
from django.db.models import Avg, Sum
from django.db.models import Q
from django.contrib.auth.models import User
from django.contrib.auth import login
from accounts.models import Reg
    ...
    def post(self, request):
        if request.user.is_authenticated:
            ...
            project = Projects.objects.create(title=data['title'], details=data['details'], category=data['category'],
                                              totalTarget=data['totaltarget'], start=data['start'], end=data['end'],
                                              user=request.user)
...
class LoginApi(APIView):
    def post(self, request):
        try:
            ...
            elif Reg.objects.get(user=user).password == request.POST['password']:
                login(user)
                return HttpResponse(status=200)
            else:
